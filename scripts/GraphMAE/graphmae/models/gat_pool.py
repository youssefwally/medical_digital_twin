...
from torch_geometric.nn import TopKPooling, SAGPooling, global_sort_pool
...
class GAT_POOL(nn.Module):
    def __init__(self,
        ...
        #     self.norms = None
        if(num_features > 0):
            # self.pool_layer = TopKPooling(in_channels=out_dim, ratio=num_features)
            self.sortpool = SortPooling(k=num_features)  # create a sort pooling layer
    ...
    def forward(self, g, inputs, return_hidden=False):
        ...
        for l in range(self.num_layers):
            ...
            # h = h.flatten(1)
        if(self.num_features > 0 and self.encoding):
            h = self.sortpool(g, h)
        # output projection
        if return_hidden:
